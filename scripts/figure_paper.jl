...
push!(scene, Entity(vehstate, VehicleDef(), EGO_ID))
vehstate = vehicle_state(80.0, main_lane(mdp.env), 10.0, mdp.env.roadway)
push!(scene, Entity(vehstate, VehicleDef(), 2))
vehstate = vehicle_state(107.0, main_lane(mdp.env), 10.0, mdp.env.roadway)
push!(scene, Entity(vehstate, VehicleDef(), 3))

c = AutomotiveVisualization.render(scene, mdp.env.roadway, 
            SceneOverlay[
...
c = AutomotiveVisualization.render([ArrowCar([0.,0.], color=RGBA(0.8, 0.8, 0.8, 1))])
